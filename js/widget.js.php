...
function showWidgetComment(fk_object,type_object, sub_object) {

	$div=$('<div title="<?php echo $langs->trans('Network') ?>" />');
	
	$div.append('<iframe src="<?php echo dol_buildpath('/network/comments.php',1) 
	?>?fk_object='+fk_object+'&type_object='+type_object+'&sub_object='+sub_object+'" width="100%" height="100%" frameborder="0"></iframe>');
...
function drawNetworkWidget(id, element, ref,$div) {
	...
	function addComment() {
		...
		$.ajax({
			...
			$div.find('input[name=comment]').val("");
		});
...
function showSociogram(id,element,ref) {
...
function setTextTag(element,ref,id) {
